...
def check_keydown_events(event, ai_settings, stats, play_button, screen, ship, bullets):
    """响应按键"""
...
def check_event(ai_settings, stats, play_button, screen, sb, ship, aliens, bullets):
    """响应按键和鼠标事件"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keydown_events(event, ai_settings, stats, play_button, screen, ship, bullets)
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    number_aliens_y = get_number_aliens_y(ai_settings, ship_height, alien_height)
...
def check_fleet_edges(ai_settings, aliens):
    ...
    for alien in aliens.sprites():
        if alien.check_edge():
            change_directions(ai_settings, aliens)
            break


def change_directions(ai_settings, aliens):
    """将整群外星人下移， 并改变它们的方向"""
    for alien in aliens.sprites():
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    """如果有子弹击中了外星，就删除相应的子弹和外星人"""
    collisions = pygame.sprite.groupcollide(bullets, aliens, True, True)

    if collisions:
        for alien in collisions.values():
            stats.score += ai_settings.alien_score * len(alien)
            sb.prep_score()
